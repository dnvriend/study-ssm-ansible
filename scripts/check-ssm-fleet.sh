...
# Check expected instance count (2 web + 2 app + 1 bastion = 5)
EXPECTED_COUNT=5

if [ "${TOTAL_INSTANCES}" -lt "${EXPECTED_COUNT}" ]; then
    echo -e "${YELLOW}WARNING: Expected ${EXPECTED_COUNT} instances, found ${TOTAL_INSTANCES}${NC}"
fi
...
if [ "${ONLINE_COUNT}" -eq "${TOTAL_INSTANCES}" ]; then
    ...
    for INSTANCE_ID in $(echo "${INSTANCES}" | jq -r '.InstanceInformationList[].InstanceId'); do
        TAGS=$(aws ssm list-tags-for-resource \
            --resource-type "ManagedInstance" \
            --resource-id "${INSTANCE_ID}" \
            --region "${AWS_REGION}" \
            --output json)

        ROLE_TAG=$(echo "${TAGS}" | jq -r '.TagList[] | select(.Key=="Role") | .Value' || echo "N/A")
        ENV_TAG=$(echo "${TAGS}" | jq -r '.TagList[] | select(.Key=="Environment") | .Value' || echo "N/A")

        echo "  ${INSTANCE_ID}: Role=${ROLE_TAG}, Environment=${ENV_TAG}"
    done
    echo ""
